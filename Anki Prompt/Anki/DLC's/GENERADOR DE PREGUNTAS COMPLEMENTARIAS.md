...
- Distribución Bloom nueva: [R%, C%, A%]
...
**Métricas banco expandido:** total [N_prev + N_nuevas]; Bloom [R%, C%, A%] (delta vs target [máx%]); Cuota [E%, T%, S%, M%]; confianza promedio [0.XX] (vs prev [0.XX]); NO_CUBIERTOS residuales [N, lista nombres]
...
**BANCO ORIGINAL:**
Preguntas: [N]
Bloom: Recordar [N, %], Comprender [N, %], Aplicar [N, %]
...
Resultado esperado: 50% preguntas banco expandido con Comprender/Aplicar
```

### Escenario 3: Banco desbalanceado en Bloom

```
Usuario ve informe con Recordar 55%, Comprender 20%, Aplicar 25% (target 40/30/30)
...
Bloom_target = Aplicar (reforzar hasta alcanzar 30%)
```
